...
func TestRanksNeverHoldsTheEmptyRank(t *testing.T) {
	t.Parallel()

	for _, configured := range []gouncer.Ranks{{""}, {"admin", ""}, {"admin"}} {
		if configured.Holds("") {
			t.Errorf("Ranks%v.Holds(\"\") = true, want an unranked account never privileged", configured)
		}
	}
}
